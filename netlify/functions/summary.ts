...
export const handler: Handler = async (event) => {
  ...
  try {
    ...
    const body: SummaryBody = JSON.parse(rawBody);
    ...
    console.error("[netlify/summary] error:", err instanceof Error ? err.message : err);
    return {
      statusCode: 502,
      body: JSON.stringify({ error: "AI summary temporarily unavailable" }),
    };
  }
};
